...
/**
 * When `expandable` is set, each row becomes a click target that opens a
 * detail modal. Left off (e.g. on the home page) the list is display-only.
 */
export default function Services({ expandable = false }: { expandable?: boolean }) {
  const [hovered, setHovered] = useState<number | null>(null);
  const [active, setActive] = useState<Service | null>(null);

  return (
    <section id="services" aria-label="Services" className="relative py-20 sm:py-40">

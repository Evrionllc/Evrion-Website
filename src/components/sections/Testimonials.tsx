...
export default function Testimonials() {
  ...
  return (
    <section aria-label="Testimonials" className="relative overflow-hidden py-20 sm:py-40">
      {/* ambient glow behind the quote */}
      <div
        aria-hidden="true"
        className="pointer-events-none absolute top-1/2 left-1/2 h-[600px] w-[900px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-iris/10 blur-[120px] will-change-transform"
      />
        ...
        <div className="mt-12 flex items-center justify-center gap-4">
          ...
          <div className="flex items-center gap-2" role="tablist" aria-label="Testimonial slides">
            {TESTIMONIALS.map((_, i) => (
              // 44px-tall tap target; the negative inline margin keeps the dots
              // visually as tightly spaced as before (desktop unchanged).
              <button
                key={i}
                type="button"
                role="tab"
                ...
                className="group/dot -mx-1.5 flex h-11 items-center justify-center px-1.5"
              >
                <span
                  className={`block h-1.5 rounded-full transition-all duration-400 ${
                    i === index ? "w-8 bg-mint" : "w-1.5 bg-faint group-hover/dot:bg-muted"
                  }`}
                />
              </button>
            ))}

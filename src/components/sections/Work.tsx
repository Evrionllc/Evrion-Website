...
type WorkProps = {
  /** cap the number of projects shown (homepage shows a teaser of 3) */
  limit?: number;
  /** render a "view all work" link beneath the grid */
  showAllLink?: boolean;
};

export default function Work({ limit, showAllLink }: WorkProps) {
  const projects = limit ? PROJECTS.slice(0, limit) : PROJECTS;

  return (
    <section id="work" aria-label="Featured work" className="relative py-20 sm:py-40">
      <div className="container-x">
        ...
        {showAllLink && (
          <div className="mt-20 flex justify-center sm:mt-28">
            <Link
              href="/work"
              className="group inline-flex min-h-11 items-center gap-2 font-mono text-xs tracking-[0.2em] text-muted uppercase transition-colors hover:text-foreground"
            >
              View all work
